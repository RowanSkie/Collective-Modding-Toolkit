...
RUNTIME_VERSION_1_1_29   = MAKE_EXE_VERSION(1, 1, 29)   # 0x010101D0    initial version released on steam
RUNTIME_VERSION_1_1_30   = MAKE_EXE_VERSION(1, 1, 30)	# 0x010101E0	day1 patch to fix xaudio problem
RUNTIME_VERSION_1_2		 = MAKE_EXE_VERSION(1, 2, 33)	# 0x01020210	beta, removed mini-mod-manager from the launcher
RUNTIME_VERSION_1_2_37	 = MAKE_EXE_VERSION(1, 2, 37)	# 0x01020250	beta - unclear
RUNTIME_VERSION_1_3_45	 = MAKE_EXE_VERSION(1, 3, 45)	# 0x010302D0	beta - compiled with optimization enabled (finally), added hbao+ and cuda-based impact particles (shame)
RUNTIME_VERSION_1_3_47	 = MAKE_EXE_VERSION(1, 3, 47)	# 0x010302F0	release, originally pushed as beta
RUNTIME_VERSION_1_4_124  = MAKE_EXE_VERSION(1, 4, 124)	# 0x010407C0	beta - preliminary internal mod manager
RUNTIME_VERSION_1_4_125  = MAKE_EXE_VERSION(1, 4, 125)	# 0x010407D0	beta - released same day as previous version. about 300K smaller
...
RUNTIME_VERSION_1_5_141	 = MAKE_EXE_VERSION(1, 5, 141)	# 0x010508D0	beta - survival mode improvements
RUNTIME_VERSION_1_5_147  = MAKE_EXE_VERSION(1, 5, 147)	# 0x01050930	beta - survival mode improvements
RUNTIME_VERSION_1_5_151  = MAKE_EXE_VERSION(1, 5, 151)	# 0x01050970	beta - survival mode improvements
RUNTIME_VERSION_1_5_154	 = MAKE_EXE_VERSION(1, 5, 154)	# 0x010509A0	beta - public editor beta
RUNTIME_VERSION_1_5_157	 = MAKE_EXE_VERSION(1, 5, 157)	# 0x010509D0	beta - released the same day
RUNTIME_VERSION_1_5_205  = MAKE_EXE_VERSION(1, 5, 205)	# 0x01050CD0	beta - released to main
RUNTIME_VERSION_1_5_210  = MAKE_EXE_VERSION(1, 5, 210)	# 0x01050D20	beta - no whatsnew
RUNTIME_VERSION_1_5_307	 = MAKE_EXE_VERSION(1, 5, 307)	# 0x01051330	release - exe almost identical
RUNTIME_VERSION_1_5_412  = MAKE_EXE_VERSION(1, 5, 412)	# 0x010519C0	beta - that's a big number. added GetOrbisModInfo console cmd (does nothing)
RUNTIME_VERSION_1_5_414  = MAKE_EXE_VERSION(1, 5, 414)	# 0x010519E0	beta - why so many releases all at once
RUNTIME_VERSION_1_5_416  = MAKE_EXE_VERSION(1, 5, 416)	# 0x01051A00	release - why so many releases all at once
...
RUNTIME_VERSION_1_6_9	 = MAKE_EXE_VERSION(1, 6, 9)	# 0x01060090	release - no interim beta
...
RUNTIME_VERSION_1_7_10	 = MAKE_EXE_VERSION(1, 7, 10)	# 0x010700A0	release - no interim beta
RUNTIME_VERSION_1_7_12	 = MAKE_EXE_VERSION(1, 7, 12)	# 0x010700C0	release - no interim beta, wtf bethesda
RUNTIME_VERSION_1_7_15	 = MAKE_EXE_VERSION(1, 7, 15)	# 0x010700F0	release - no interim beta, released on a holiday weekend, wtf bethesda
...
RUNTIME_VERSION_1_8_7	 = MAKE_EXE_VERSION(1, 8, 7)	# 0x01080070	release - poking at the built-in mod manager
RUNTIME_VERSION_1_9_4	 = MAKE_EXE_VERSION(1, 9, 4)	# 0x01090040	release - high-resolution texture pack
RUNTIME_VERSION_1_10_20	 = MAKE_EXE_VERSION(1, 10, 20)	# 0x010A0140	beta/release - creation club
RUNTIME_VERSION_1_10_26	 = MAKE_EXE_VERSION(1, 10, 26)	# 0x010A01A0	creation club update 2
RUNTIME_VERSION_1_10_40	 = MAKE_EXE_VERSION(1, 10, 40)	# 0x010A0280	creation club update 3 (thanks for cleaning up plugin identification)
...
RUNTIME_VERSION_1_10_82	 = MAKE_EXE_VERSION(1, 10, 82)	# 0x010A0520	creation club update 7 (startup speed?)
...
RUNTIME_VERSION_1_10_106 = MAKE_EXE_VERSION(1, 10, 106)	# 0x010A06A0	creation club update 10 (no addresses changed)
RUNTIME_VERSION_1_10_111 = MAKE_EXE_VERSION(1, 10, 111)	# 0x010A06F0	creation club update 11 (no addresses changed)
RUNTIME_VERSION_1_10_114 = MAKE_EXE_VERSION(1, 10, 114)	# 0x010A0720	creation club update 12 (no addresses changed)
RUNTIME_VERSION_1_10_120 = MAKE_EXE_VERSION(1, 10, 120)	# 0x010A0780	creation club update 13 (no addresses changed)
...
class F4SEPluginVersionData(ctypes.Structure):
    ...
    def HasSupportAddressLibraryAE(self):
        return bool(F4SEPluginAddressIndependence(self.addressIndependence) & F4SEPluginAddressIndependence.kAddressIndependence_AddressLibrary_1_11_137)

    def HasSupportAddressLibraryNG(self):
        return bool(F4SEPluginAddressIndependence(self.addressIndependence) & F4SEPluginAddressIndependence.kAddressIndependence_AddressLibrary_1_10_980)

    def HasSupportSignatureScanning(self):
        return bool(F4SEPluginAddressIndependence(self.addressIndependence) & F4SEPluginAddressIndependence.kAddressIndependence_Signatures)

    def HasSupportStructureIndependenceAE(self):
        return bool(F4SEPluginStructureIndependence(self.structureIndependence) & F4SEPluginStructureIndependence.kStructureIndependence_1_11_137Layout)

    def HasSupportStructureIndependenceNG(self):
        return bool(F4SEPluginStructureIndependence(self.structureIndependence) & F4SEPluginStructureIndependence.kStructureIndependence_1_10_980Layout)

    def HasSupportNoStructureIndependence(self):
        return bool(F4SEPluginStructureIndependence(self.structureIndependence) & F4SEPluginStructureIndependence.kStructureIndependence_NoStructs)
    ...
    def IsSupportVersion(self, version: ctypes.c_uint32):
        ...
        # simple version list
        if (not hasVersionIndependent) and (not self.HasCompatibleCurrentVersion(version)):
            return False
    ...
    def IsSupportVersionNG(self):
        ...
        result = (self.IsSupportVersion(n) for n in (
...
class PluginDllInfo(ctypes.Structure):
    _fields_ = [
        ("IsF4SE", ctypes.c_bool),
        ("SupportsOG", ctypes.c_bool),
        ("SupportsNGAE", ctypes.c_bool),
        ("SupportsNG", ctypes.c_bool),
        ("SupportsAE", ctypes.c_bool),
        ("AddrIndependentNG", ctypes.c_bool),
        ("StructIndependentNG", ctypes.c_bool),
        ("AddrIndependentAE", ctypes.c_bool),
        ("StructIndependentAE", ctypes.c_bool),
        ("SupportsCurrent", ctypes.c_bool),
    ]

    def __init__(self, file_path: Path, version: ctypes.c_uint32):
        dll = ctypes.WinDLL(str(file_path), winmode=DONT_RESOLVE_DLL_REFERENCES)
        # fixed IsF4SE if exists any load functions, query function have only og, ngae have version data
        self.IsF4SE = hasattr(dll, "F4SEPlugin_Load") or hasattr(dll, "F4SEPlugin_Preload")
        self.SupportsOG = hasattr(dll, "F4SEPlugin_Query")
        self.SupportsNGAE = hasattr(dll, "F4SEPlugin_Version")

        if self.SupportsNGAE:
            # Get info about plugin from dll
            v = ctypes.cast(dll.F4SEPlugin_Version, ctypes.POINTER(F4SEPluginVersionData),).contents

            # Output debug info plugin from dll
            # Sets P71_DEBUG_F4SE_LIBRARY as 0 - disabled | 1 - enabled
            if P71_DEBUG_F4SE_LIBRARY:
                print(f"dataVersion: {v.dataVersion:>20}")
                print(f"pluginVersion: {v.pluginVersion:>23}")
                print(f"name: {v.name!s:>46}")
                print(f"author: {v.author!s:>36}")
                print(f"addressIndependence: {v.addressIndependence:>12}")
                print(f"structureIndependence: {v.structureIndependence:>10}")
                print("compatibleVersions:", "".rjust(11), *v.compatibleVersions)

            # checks NG supports
            self.AddrIndependentNG = v.HasSupportAddressLibraryNG() or v.HasSupportSignatureScanning()
            self.StructIndependentNG = v.HasSupportStructureIndependenceNG() or v.HasSupportNoStructureIndependence()

            if self.AddrIndependentNG and self.StructIndependentNG:
                # If it uses Address Library or signatures
                self.SupportsNG = True
            else:
                # Check known NG versions
                self.SupportsNG = v.IsSupportVersionNG()

            self.AddrIndependentAE = v.HasSupportAddressLibraryAE() or v.HasSupportSignatureScanning()
            self.StructIndependentAE = v.HasSupportStructureIndependenceAE() or v.HasSupportNoStructureIndependence()
            
            if self.AddrIndependentAE and self.StructIndependentAE:
                # If it uses Address Library or signatures
                self.SupportsAE = True
            else:
                # Check known NG versions
                self.SupportsAE = v.IsSupportVersionAE()

            self.SupportsCurrent = v.IsSupportVersion(version)
...
if P71_DEBUG_F4SE_LIBRARY:
    ...
    PrintDebug(r"G:/SteamLibrary/steamapps/common/Fallout 4/Data/f4se/plugins/HighFPSPhysicsFix.dll", RUNTIME_VERSION_1_11_221)